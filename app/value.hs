...
instance JSON Value where
  ...
  readJSON (JSObject obj) = case fromJSObject obj of
    (("cons", jsv) : []) -> (readJSON jsv) >>= (return . Cons)
...
instance JSON Control where
  ...
  readJSON (JSString jss) = case fromJSString jss of
    name@('?' : _) -> Ok $ Function name
    name@('!' : _) -> Ok $ Action name
    name -> Ok $ Primitive name
  readJSON (JSObject obj) =
    let pairs = fromJSObject obj
     in do
          params <- property "parameters" pairs
