...
concretize pstore (Symbol addr) = case load addr pstore of
  (Result dta) -> Right dta
  _ -> Left addr
...
apply (Closure ids expr env) vals kont heap@(bstore, cstore, pstore)
  ...
  | otherwise =
      let (env2, bstore2) = new (zip ids vals) env bstore
       in Jump $ Ongoing expr env2 kont (bstore2, cstore, pstore)

applyKontinuation :: Kontinuation -> Value -> Heap -> Transition
applyKontinuation (K.Application [] vals _ kont) val heap = case vals ++ [val] of
  ((Control ctr) : args) -> apply ctr args kont heap
...
applyKontinuation (K.Assignment id env kont) val heap@(bstore, cstore, pstore) =
  maybe
    (crash ("no such identifier: " ++ id) heap)
...
applyKontinuation (K.Branch expr1 expr2 env kont) val heap@(_, _, pstore) = case concretize pstore val of
  (Left symbol) -> Fork symbol expr1 expr2 env kont heap
...
applyFunction "?string->data" ((DString str) : []) = case decode str of
  (Ok dta) -> dta
  (Error _) -> DNull
